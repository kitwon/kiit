...
var app = {
	...
	initUI: function() {
		...
		$('.translate-wrap').css('-webkit-transform', 'translate3d(0,' + _this.data.screenHeight + 'px, 0)');
		...
		$('.main-sun').show().css('-webkit-transform', 'translate3d(' + _this.data.boxTransX + 'px, ' + _this.data.boxTtransY + 'px, 0)');
	...
	animate: function(translateVal) {
		...
		$('.translate-wrap').css('-webkit-transform', 'translate3d(0,' + translateVal + 'px, 0)');

		if (translateVal === 0) {
			setTimeout("$('body').css('overflow-y', 'auto')", _this.data.transitionTime);
			$('.main-sun').css('-webkit-transform', 'translate3d(' + _this.data.btnTransX + 'px, ' + _this.data.btnTtransY + 'px, 0) scale(0.7, 0.7)');
			$('.nav-button').css('-webkit-transform', 'scaleX(1)');
			requestFrame.call(window, _this.loopPath);
		} else {
			$('body').css('overflow-y', 'hidden')
			$('.main-sun').css('-webkit-transform', 'translate3d(' + _this.data.boxTransX + 'px, ' + _this.data.boxTtransY + 'px, 0) scale(1, 1)');
		...
		setTimeout(function() {
			if(_this.data.wrapWidth > $('.translate-wrap').width()) {
				$('.main-sun').css('-webkit-transform', 'translate3d(' + (_this.data.btnTransX - 16) + 'px, ' + _this.data.btnTtransY + 'px, 0) scale(0.7, 0.7)');
			}
